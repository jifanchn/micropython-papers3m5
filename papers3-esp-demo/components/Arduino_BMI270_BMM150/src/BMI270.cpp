...
static int8_t test_bmi2_set_config2(struct bmi2_dev *bmi2_dev)
{
...
void BoschSensorClass::test()
{
  /* Variable to define result */
    ...
    if (rslt == BMI2_OK)
    {
        ...
        rslt = test_bmi2_set_config2(&bmi2);

        bmi2_remap remapped_axis = {0};
        // remapped_axis.x = BMI2_Y;
        // remapped_axis.y = BMI2_X;
        // remapped_axis.z = BMI2_Z;
        remapped_axis.x = BMI2_Y;
        remapped_axis.y = BMI2_NEG_Z;
        remapped_axis.z = BMI2_X;

        rslt = bmi2_set_remap_axes(&remapped_axis, &bmi2);
        ...
        if (rslt == BMI2_OK)
        {
            ...
            if (rslt == BMI2_OK)
            {
                ...
                for (;;)
                {
                    ...
                    {
                      x++;
                        printf("Wrist wear wakeup detected %d\n", x);
                    }
                    vTaskDelay(1);
                }
            }
...
int BoschSensorClass::wristWearWakeUpAvailable()
{
  uint16_t status;
