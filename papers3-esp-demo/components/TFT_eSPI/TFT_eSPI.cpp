...
void TFT_eSPI::setTextSize(uint8_t s)
{
  ...
  if (_is_freetype_loaded && _use_freetype_font) {
      if (_render_map.count(s) != 0) {
          ...
          log_e("Freetype: Size %d not found.", s);
...
uint8_t TFT_eSPI::alphaBlend4Bit(uint8_t alpha, uint8_t fgc, uint8_t bgc) {
    static const uint8_t alphamap[16] = {0,   17,  34,  51,  68,  85,  102, 119,
...
#ifdef FREETYPE_FONT
    if ((gfxFont == NULL) && _is_freetype_loaded && _use_freetype_font) {
        if ((_render_map.count(_ft_font_size) == 0) ||
            (_current_ft_render == nullptr)) {
        ...
        if (textdatum == TL_DATUM || textdatum == TC_DATUM ||
            textdatum == TR_DATUM) {
            base_y = poY + _current_ft_render->origin;
            fill_y = poY;
        } else if (textdatum == CL_DATUM || textdatum == CC_DATUM ||
                   textdatum == CR_DATUM) {
            base_y = poY + _current_ft_render->origin -
                     (_current_ft_render->pixel_size >> 1);
            fill_y = poY - (_current_ft_render->pixel_size >> 1);
        } else if (textdatum == BL_DATUM || textdatum == BC_DATUM ||
                   textdatum == BR_DATUM) {
            base_y = poY + _current_ft_render->origin -
                     _current_ft_render->pixel_size;
            fill_y = poY - _current_ft_render->pixel_size;
        } else  // baseline
        {
            base_y = poY;
            fill_y = poY - _current_ft_render->origin;
        }

        bool is_fill_bg = false;
        if (textcolor != textbgcolor) {
            is_fill_bg = true;
        }

        uint16_t fill_h = 0, auto_fill_margin = 0;
        if (is_fill_bg) {
            if (_fill_margin_active) {
                fill_h = _current_ft_render->pixel_size + _fill_margin_bottom +
                         _fill_margin_top;
            } else {
                auto_fill_margin = _current_ft_render->pixel_size >> 3;
                fill_h =
                    _current_ft_render->pixel_size + (auto_fill_margin << 1);
            }
        }

        // Left/right alignment does not require pre-rendering
        if (textdatum == TL_DATUM || textdatum == CL_DATUM ||
            textdatum == BL_DATUM) {
            if (is_fill_bg) {
                if (_fill_margin_active) {
                    fillRect(poX - _fill_margin_left, fill_y - _fill_margin_top,
                             _fill_margin_left, fill_h, textbgcolor);
                } else {
                    fillRect(poX - auto_fill_margin, fill_y - auto_fill_margin,
                             auto_fill_margin, fill_h, textbgcolor);
                }
            }
            while (n < len) {
                if (poX > width()) {
                ...
                if (is_fill_bg) {
                    if (_fill_margin_active) {
                        fillRect(poX, fill_y - _fill_margin_top,
                                 _current_ft_render->advance, fill_h,
                                 textbgcolor);
                    } else {
                        fillRect(poX, fill_y - auto_fill_margin,
                                 _current_ft_render->advance, fill_h,
                                 textbgcolor);
                    }
                }
                drawFreetypeBitmap(poX + _current_ft_render->bitmap_left,
                                   base_y - _current_ft_render->bitmap_top,
                                   _current_ft_render->bitmap_width,
                                   _current_ft_render->bitmap_height,
                                   _current_ft_render->bitmap);
                poX += _current_ft_render->advance;
                sumX += _current_ft_render->advance;
            }
            if (is_fill_bg) {
                if (_fill_margin_active) {
                    fillRect(poX, fill_y - _fill_margin_top, _fill_margin_right,
                             fill_h, textbgcolor);
                } else {
                    fillRect(poX, fill_y - auto_fill_margin, auto_fill_margin,
                             fill_h, textbgcolor);
                }
            }
            return sumX;
        ...
        if (textdatum == TR_DATUM || textdatum == CR_DATUM ||
            textdatum == BR_DATUM) {
            uint16_t uniCodes[len];
            uint16_t unicode_num = 0;
            while (n < len) {
                uniCodes[unicode_num] =
                    decodeUTF8((uint8_t *)string, &n, len - n);
                unicode_num++;
            }
            ...
            for (int i = unicode_num - 1; i >= 0; i--) {
                if (poX < 0) {
                    return sumX;
                }

                if (font_render_glyph(_current_ft_render, uniCodes[i]) !=
                    ESP_OK) {
                    log_e("Font render faild.");
                    return sumX;
                }
        ...
        // Center alignment requires pre-rendering, which needs to temporarily
        // occupy some memory
        uint16_t cwidth      = 0;
        uint16_t unicode_num = 0;
        n                    = 0;
        ...
        n            = 0;
        uint16_t idx = 0;
        ft_bitmap_cache_t tempcache[unicode_num];
        uint32_t bitmap_size = 0;
        while (n < len) {
            tempcache[idx].bitmap  = nullptr;
            tempcache[idx].unicode = decodeUTF8((uint8_t *)string, &n, len - n);
            tempcache[idx].shared_flag = 0;
            for (int i = 0; i < idx; i++) {
                if (tempcache[idx].unicode == tempcache[i].unicode) {
                    tempcache[idx]             = tempcache[i];
                    tempcache[idx].shared_flag = 1;
                    goto _label_done;
                }
            }

            if (font_render_glyph(_current_ft_render, tempcache[idx].unicode) !=
                ESP_OK) {
                log_e("Font render faild, unicode = %04X.",
                      tempcache[idx].unicode);
                return sumX;
            }
            bitmap_size = ((_current_ft_render->bitmap_height *
                            _current_ft_render->bitmap_width) >>
                           1) +
                          1;
            tempcache[idx].bitmap = (uint8_t *)malloc(bitmap_size);
            if (tempcache[idx].bitmap == nullptr) {
                log_e("Out of memory");
                for (int i = 0; i < unicode_num; i++) {
                    if (tempcache[i].bitmap != nullptr) {
                        free(tempcache[i].bitmap);
                    }
                }
                return sumX;
            }
            memcpy(tempcache[idx].bitmap, _current_ft_render->bitmap,
                   bitmap_size);
            tempcache[idx].advance       = _current_ft_render->advance;
            tempcache[idx].bitmap_height = _current_ft_render->bitmap_height;
            tempcache[idx].bitmap_width  = _current_ft_render->bitmap_width;
            tempcache[idx].bitmap_left   = _current_ft_render->bitmap_left;
            tempcache[idx].bitmap_top    = _current_ft_render->bitmap_top;

        _label_done:;
            cwidth += tempcache[idx].advance;
            idx++;
        }
        ...
        if (is_fill_bg) {
            if (_fill_margin_active) {
                fillRect(poX - _fill_margin_left, fill_y - _fill_margin_top,
                         cwidth + _fill_margin_left + _fill_margin_right,
                         fill_h, textbgcolor);
            } else {
                fillRect(poX - auto_fill_margin, fill_y - auto_fill_margin,
                         cwidth + (auto_fill_margin << 1), fill_h, textbgcolor);
            }
        }
        ...
        for (int i = 0; i < unicode_num; i++) {
            if (tempcache[i].bitmap != nullptr) {
                drawFreetypeBitmap(
                    poX + tempcache[i].bitmap_left,
                    base_y - tempcache[i].bitmap_top, tempcache[i].bitmap_width,
                    tempcache[i].bitmap_height, tempcache[i].bitmap);
                poX += tempcache[i].advance;
                if (tempcache[i].shared_flag) {
                    continue;
                }
                free(tempcache[i].bitmap);
            }
...
void TFT_eSPI::setTextFillMargin(uint16_t left, uint16_t right, int16_t top,
                                 int16_t bottom) {
    _fill_margin_left   = left;
    _fill_margin_right  = right;
    _fill_margin_top    = top;
    _fill_margin_bottom = bottom;
    _fill_margin_active = true;
}

void TFT_eSPI::setTextLineSpace(uint8_t space) {
    _ft_linespace = space;
}
...
esp_err_t TFT_eSPI::unloadFont() {
    for (auto iter_render = _render_map.begin();
         iter_render != _render_map.end(); iter_render++) {
...
esp_err_t TFT_eSPI::createRender(uint16_t size, uint16_t cache_size) {
    ...
    if (font_render_init(&font_render, &_font_face, size, cache_size) !=
        ESP_OK) {
    ...
    font_render.origin = font_render.pixel_size - font_render.bitmap_height +
                         font_render.bitmap_top;
...
void TFT_eSPI::drawFreetypeBitmap(int32_t cx, int32_t cy, uint16_t bw,
...
esp_err_t TFT_eSPI::preRender(uint16_t code) {
    return font_render_glyph(_current_ft_render, code);
}

void TFT_eSPI::drawFreetypeGlyph(uint16_t code) {
    if (_current_ft_render == nullptr) {
        return;
    }

    if (code == '\n') {
        cursor_x = _textarea_x;
        cursor_y += _current_ft_render->pixel_size + _ft_linespace;
        if (textwrapY && cursor_y + _current_ft_render->pixel_size >=
                             _textarea_y + _textarea_h) {
            cursor_y = _textarea_y;
        }
        return;
    }
    ...
    if (textwrapX && (this->cursor_x + _current_ft_render->advance >
                      (_textarea_x + _textarea_w))) {
        this->cursor_y += _current_ft_render->pixel_size + _ft_linespace;
        this->cursor_x = _textarea_x;
    }
    if (textwrapY && (this->cursor_y + _current_ft_render->pixel_size >=
                      (_textarea_y + _textarea_h))) {
        this->cursor_y = _textarea_y;
    }
    if (cursor_x == _textarea_x) cursor_x -= _current_ft_render->bitmap_left;

    int16_t cy =
        cursor_y - _current_ft_render->bitmap_top + _current_ft_render->origin;
    int16_t cx = cursor_x + _current_ft_render->bitmap_left;

    drawFreetypeBitmap(cx, cy, _current_ft_render->bitmap_width,
                       _current_ft_render->bitmap_height,
                       _current_ft_render->bitmap);

    cursor_x += _current_ft_render->advance;
}
